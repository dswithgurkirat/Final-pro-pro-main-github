...
function exportAnx7PDF() {
  ...
  const extractData = (tableId) => {
    ...
    tbl.querySelectorAll('tbody tr').forEach(tr => {
      ...
      for (let i = 0; i < tds.length - 1; i++) {
        row.push(getCellTextAnx7(tds[i]));
      }
      rows.push(row);
    });
    return { headers, rows };
  ...
  doc.setFontSize(12);
  doc.setTextColor(0, 0, 0);
  doc.text("Annexure-VII", pageWidth - 40, 55, { align: "right" });

  doc.setFont("times", "bold");
  doc.setFontSize(11);
  doc.text("> Final Patta Land Summary:", 40, startY);
  startY += 15;

  const tableData = extractData('anx7-patta-final');
  doc.autoTable({
    startY: startY,
  ...
  doc.save('Annexure_VII_Final_Patta_Lands.pdf');
...
function renderPdfUploadUIAnx7() {
  ...
  const dlBtn = document.getElementById('anx7-download-btn');
  const delBtn = document.getElementById('anx7-delete-btn');
  const previewBtn = document.getElementById('anx7-preview-btn');
  const previewSection = document.getElementById('pdf-preview-section-anx7');
  ...
  const pdfName = S.activeProject.anx7PdfName;
  const canEdit = (S.role === 'user');
  const uploadInput = document.getElementById('anx7-pdf-input');
  if (uploadInput) {
  ...
  if (!pdfName) {
    ...
    if (previewSection && previewSection.style.display === 'block' && iframe) {
      if (S.activeProject.pdfData && S.activeProject.pdfData.anx7) {
        if (iframe.src !== S.activeProject.pdfData.anx7) {
...
function togglePDFPreviewAnx7() {
  ...
  if (previewSection.style.display === 'block') {
    ...
    if (S.activeProject && S.activeProject.pdfData && S.activeProject.pdfData.anx7) {
      iframe.src = S.activeProject.pdfData.anx7;
...
async function deletePdfAnx7() {
  ...
  if (pIdx !== -1) {
    S.projects[pIdx].anx7PdfName = null;
    if (S.projects[pIdx].pdfData) S.projects[pIdx].pdfData.anx7 = null;
  }
...
function handlePDFUploadAnx7(event) {
  ...
  const pIdx = S.projects.findIndex(p => p.id === S.activeProject.id);
  if (pIdx !== -1) {
    S.projects[pIdx].anx7PdfName = file.name;
    if (!S.projects[pIdx].pdfData) S.projects[pIdx].pdfData = {};
    S.projects[pIdx].pdfData.anx7 = fileURL;
  }
...
function downloadPdfAnx7() {
  ...
  if (!S.activeProject.anx7PdfName) {
